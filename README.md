...
Finally, we use the **when is undefined** statement to not generate an token if one already exist. This means we can now run the playbook as many times as we want. Only the first time the playbook runs on a Fortigate will a token be generated. This is very powerful as we can now add new Fortigates to our inventory and ensure that they have API-tokens applied, while also being certain that existing fortigates will not have their token changed unnecessarily.
