...
These are the steps to generate it the manual way. In short, we create a new **ansible** API-user and give it **prof_admin** privileges. Once created, we generate an API access-token, which we must provide in our API-requests to this Fortigate. This access-token should be finally added to our Ansible inventory file in the **ansible_httpapi_session_key** variable:
```
...
But this is an Ansible course after all, we want to automate all the things! In that spirit, I have provided a playbook to do this job for us. Let's examine its contents:
...
The third new "concept" is the **loop**. Let's pretend that our lab network has more than one Fortigate. To make sure that we generate API access-tokens for all Fortigates, I use the **groups.FORTIOS** variable to get a list of all Fortigates that belong to the FORTIOS group in our inventory file.  
While looping over each member of the group, we save the name of the Fortigate to a variable **fgt** and then run the **include_role** task. So when we run the **generate_fortios_access_token** role, we pass along the **fgt** variable. In this case we only iterate over the loop once, because there is only one Fortigate in the list. The value of **fgt** in this instance is **FW-1**.
...
In short, we can access the management IP-address of FW-1 by looking at **hostvars["FW-1"]['ansible_host']**. By wrapping this variable in **{{ }}** curly brackets, we tell Ansible to replace **hostvars[...** with 172.20.20.4. So the spawn command effectively becomes **spawn ssh admin@172.20.20.4**. 
...
In the second task we loop through all lines of the registered output, searching for a line containing the "NEW API key" output. Once that line is found, we split the line every time we encounter a whitespace and fetch the last entry as our access-token. We save the access-token to variable **access_token**.
